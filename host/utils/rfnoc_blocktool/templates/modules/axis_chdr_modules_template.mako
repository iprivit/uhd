%for idx, input in enumerate(config.data.inputs):
  chdr_to_chdr_data #(
    ...
    .m_axis_chdr_tdata(m_${input.name}_chdr_tdata),
    .m_axis_chdr_tlast(m_${input.name}_chdr_tlast),
    .m_axis_chdr_tvalid(m_${input.name}_chdr_tvalid),
    .m_axis_chdr_tready(m_${input.name}_chdr_tready),
...
%for idx, output in enumerate(config.data.outputs):
  chdr_to_chdr_data #(
    ...
    .s_axis_chdr_tdata(s_${output.name}_chdr_tdata),
    .s_axis_chdr_tlast(s_${output.name}_chdr_tlast),
    .s_axis_chdr_tvalid(s_${output.name}_chdr_tvalid),
    .s_axis_chdr_tready(s_${output.name}_chdr_tready),
